...
function TargetPathInput(props) {
    ...
    useEffect(() => {
        ...
        if (!path) return;
        const val = path.includes('{') ? stringFormat(path, {
            ...moduleNames,
            moduleChineseName,
            ...projectNames,
            packageName
    ...
    }, [form, nameStr, value, moduleNames, templateId, templateOptions, moduleChineseName, projectNames, packageName]);

    // moduleNames 改变 根据模块名，处理地址
    useEffect(() => {
        const path = templateOptions.find(item => item.value === templateId)?.record?.targetPath;
        const val = stringFormat(path, {...moduleNames, moduleChineseName, ...projectNames, packageName});
        const name = nameStr.split(',');
        name[1] = window.parseInt(name[1], 10);
        form.setFields([{name, value: val}]);
    }, [form, moduleNames, nameStr, templateId, templateOptions, moduleChineseName, projectNames, packageName]);
    ...
    return (
        <div>
            ...
            <span style={{marginLeft: 4}}>
                {exist ? <ExclamationCircleOutlined style={{color: 'orange'}}/> :
