...
export default config({
    ...
    const handleSubmit = useCallback(async (values) => {
        ...
        if (isEdit) {
            await props.ajax.put('/teams', params, { setLoading, successTip: '修改成功！' });
        } else {
            await props.ajax.post('/teams', params, { setLoading, successTip: '创建成功！' });
        }
    ...
    useEffect(() => {
        if (!isEdit) return;
        (async () => {
            const res = await props.ajax.get('/teams', { id: record?.id }, [], { setLoading });
